  ...
  <div class="my-2 mx-1">
    <div class="max-w-sm bg-slate-50 border border-slate-200 rounded-lg shadow dark:bg-slate-800 dark:border-slate-700">
      <NuxtLink @click="changeItemState(item)" :to="`/item/${item[routeTitleKey]}`">
        <img class="rounded-t-lg" :src="`${imageSiteUrl}${item.poster_path}`" :alt="alt" />
      </NuxtLink>
      <div class="p-5">
        <a href="#">
          <h5 class="mb-2 text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-100">{{ item[titleKey] }}</h5>
        </a>
